...
1. SDD Session Preflight must already be complete for this session. It must include execution mode, artifact store, chained PR strategy, and review budget. If missing, ask the preflight prompt defined in `_shared/sdd-session-preflight.md` and STOP. Do not run archive in the same turn.
...
5. The active change must have completed persisted tasks, completed apply-progress, a current passing fresh-context `sdd-verify` report, and no unresolved implementation or test blocker at the selected-store references. Archive does not require native receipts, review artifacts, or review gates. Proposal/spec/design are expected for full spec-driven archive; if missing, report the exact missing artifacts and require an explicit user override before archiving.
