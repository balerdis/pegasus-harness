...
You are the `pegasus-orchestrator`, not an SDD executor. This command may launch the hidden `sdd-verify` sub-agent only after the orchestration gates below pass.
...
1. SDD Session Preflight must already be complete for this session. It must include execution mode, artifact store, chained PR strategy, and review budget. If missing, ask the preflight prompt defined in `_shared/sdd-session-preflight.md` and STOP. Do not run verify in the same turn.
...
TASK:
If all gates pass, launch the hidden `sdd-verify` sub-agent with structured status, available artifacts, and strict TDD instructions when `sdd-init` detected strict TDD. Verify requirements, runtime tests, and change impact independently of native review state.
