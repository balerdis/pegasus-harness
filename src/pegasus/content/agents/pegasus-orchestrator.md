...
For structural code discovery, caller and flow analysis, impact analysis, and test targeting, use CBM and follow `{{skills_root}}/_shared/cbm-convention.md` for tool priority, the index-repair rule, and the narrow fallback conditions; state the reason whenever you fall back. If that path is missing or unreadable, say so and proceed without claiming graph evidence; do not invent your own tool order or fallback conditions.
