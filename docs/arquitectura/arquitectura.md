...
| Un `granted_directories` inválido escrito a mano en el journal deja sin usar los nueve comandos que lo leen —`install`, `update`, `uninstall`, los tres de `mcp`, los dos de `directory` y `doctor`—, incluido el `directory revoke` con el que uno querría sacarlo. La validación al cargar es correcta y no se toca: impide el replay de una escalación. Y tolerarla devolviendo el campo vacío **no** es la salida, aunque vacío sea el estado más restrictivo: el campo se reescribe en cada guardado y un valor vacío se omite al serializar, así que «cargar vacío» se volvería «borrado permanentemente en el próximo comando que toque el journal por cualquier motivo», en silencio. El fail-closed de hoy bloquea, pero nunca escribe | Una decisión de diseño sobre la carga del journal que ya no es la que esta fila pedía. Lo que falta no es tolerar el campo sino darle a la persona una forma de sacarlo sin editar JSON a mano: un modo de reparación que escriba una sola vez, a pedido explícito, en vez de degradar la lectura de todos los comandos | Correr `doctor`: desde 5.35.0 no se cae con el journal roto, nombra el archivo, repite el error —que ya dice qué campo es— y nombra los dos caminos de vuelta. Y `pegasus restore` es uno de ellos: **no** pasa por el parser del journal, reconstruye desde el manifiesto del snapshot, así que funciona con el journal corrupto —verificado corrompiéndolo a propósito y recuperándolo byte a byte—. La versión anterior de esta fila decía que no había camino de recuperación fuera de editar el JSON, y era falso |
...
| La poda de directorios no alcanza a ninguna instalación anterior a 5.28.0. `make_dir` sólo informa lo que **crea**, y en una instalación que ya existe los directorios ya están, así que `created_dirs` se queda vacío para siempre. Medido: un journal degradado a la forma anterior deja 31 directorios vacíos tras el uninstall, con `pruned` vacío. Y no hay curación: `uninstall` tampoco poda con el registro vacío, así que ni borrar y reinstalar sirve | Una decisión que no es del código: alcanzar la base instalada obliga a decidir qué hacer con un directorio del que no sabemos si es nuestro. Las dos salidas conocidas son adoptar todo directorio cuyos archivos estén todos reclamados por el journal —que no es saber quién lo creó sino que no hay nada ajeno adentro, y reintroduce la misma ambigüedad que la poda vino a cerrar— o limpiarlo con un acto explícito de la persona. Lo segundo es lo honesto y es superficie nueva. Hasta entonces la poda vale hacia adelante y punto, que es mejor que borrar por las dudas | Desde 5.35.0 `doctor` los **nombra**, sin reclamarlos ni prometer limpiarlos: dice cuántos hay y cuáles, y que esta instalación no puede confirmarlos como propios. Se borran a mano sin riesgo: están vacíos. Un punto ciego heredado de la poda real y que el reporte comparte: si el propio directorio de configuración es un symlink —dotfiles con stow o chezmoi— no se recorre nada y el reporte sale vacío en silencio, que es honesto pero menos útil de lo que el texto sugiere |
| La protección contra un cambio de esquema de `id` sigue sin cubrir la forma `append`. Un `config-key` cuyo pointer termina en `/-` queda fuera del reconocimiento por dirección a propósito, porque varios registros comparten legítimamente la misma `(target, pointer)`; la consecuencia es que un append cuyo `id` cambia se retira y no se recrea en la misma corrida, y se cura en la siguiente | No es alcanzable con el contenido de hoy: los tres append embarcados apuntan al mismo `layout.settings_file` y sus `id` son literales fijos, así que ninguno tiene un path derivado de la identidad y el guardián de dos identidades no los ve. Lo que falta es decidir si se cubre antes de que alguien agregue un append cuyo destino sí dependa de la identidad, o se acepta que el primero pierda una corrida. La otra mitad de esta deuda —que nada exigiera el nombre en el `id` de un artefacto con path derivado— quedó cerrada, y está en la tabla de abajo | Si un `append` renombrado desaparece tras un `install`, correr `install` otra vez: se recrea en la corrida siguiente |
