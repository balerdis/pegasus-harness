...
| El retiro saca archivos y deja los directorios que quedaron sin contenido. `_retire_files` borra cada `target` y nada más, así que retirar una skill entera —un directorio con archivos adentro— deja el esqueleto en pie. Medido sobre un HOME descartable: `pegasus uninstall --cli opencode` informa «removed 99» y deja **43 directorios vacíos**, incluidos `skills/`, `prompts/`, `commands/`, `plugins/` y `registry/`. No es inocuo: el registry que recorre `skills/` encuentra un `skills/<nombre>/references/` vacío y lo ve como una skill instalada. Está así desde siempre y se hizo visible en 5.9.0, la primera vez que Pegasus retira una skill completa | La regla correcta ya está clara y no necesita método nuevo en el puerto: borrar de abajo hacia arriba **sólo lo que quedó vacío** —`list_dir` para preguntarlo, `remove_dir` para sacarlo—, nunca el directorio de configuración del CLI, y deteniéndose en el primer directorio que todavía tenga algo. Eso es lo que hace que un archivo que la persona puso adentro de una carpeta de Pegasus salve su directorio y todos los de arriba, que es el mismo principio que rige el resto del producto. Lo que falta decidir no es la regla sino el alcance: `unplace` deshace un install fallido llamando a `retire`, así que el cambio altera cómo se ve una instalación abortada, y eso hay que ejercitarlo aparte en vez de asumirlo |
