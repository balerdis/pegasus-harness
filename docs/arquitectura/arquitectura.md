...
| (d) Strict TDD fuera de SDD | La regla de lectura con un solo dueño, su resolución por sesión en el brief y `N/A: no behavior changed` | En curso. d1–d4 hecho: la regla de lectura y el marker viven solo en `implementation-craft.md` (975 palabras), y `sdd-init`, `sdd-apply` y `sdd-verify` la señalan; `STRICT TDD MODE IS ACTIVE` ya no está en `src/`; un brief sin la grafía reconocida se resuelve con la regla, nunca como modo standard; ningún archivo embarcado trae un marker suelto, tampoco como encabezado ni celda de tabla; suite 3011 |
...
| `test_install_script:InterruptedDownloadLeavesNoTempDirTest` puede fallar sin que haya nada roto, y lo que queda es una carrera de bash y no del test. Lo que la fila decía antes —«intermitente bajo carga», «aislado pasa siempre»— describía el síntoma y ocultaba tres defectos distintos, los tres ya cerrados: la espera competía con un plazo fijo en vez de observar la salida del proceso; el `curl` de mentira dormía un tiempo finito, así que en una corrida lenta terminaba solo y el test medía otro camino; y **nunca se stubeaban `node` ni `opencode` como presentes**, así que el bucle que espera «a que aparezca un directorio temporal» podía engancharse al de `nvm` —que corre antes y usa el mismo `TMPDIR`— y probar el `trap` equivocado, en verde. Ese tercero se verificó instrumentando el stub para que registre sus argumentos: hoy la descarga interrumpida es la del binario del producto, comprobado. Lo que sigue abierto es el mecanismo de abajo: bash **bloquea `SIGINT` en su propia máscara** mientras espera al hijo de una sustitución de comandos, y esa máscara sobrevive a `fork()` y a `exec()`, así que una señal que cae en esa ventana deja al nieto con la señal bloqueada-pero-pendiente y el `wait()` no vuelve nunca. Medido: forzando la señal en el instante exacto en que aparece el directorio aparece en un 7-8% de las corridas; con el sondeo real del test no apareció ninguna vez en 211 corridas | Nada que esté en este repositorio: el bloqueo de la máscara es de bash. Lo que sí se cerró es que el test no puede **colgarse** por eso — un intento anterior de arreglo lo empeoró justamente así, cambiando la intermitencia por un deadlock permanente al esperar EOF sobre pipes que un nieto huérfano mantenía abiertos. Hoy la salida va a `DEVNULL`, la limpieza mata el grupo entero con `SIGKILL` —que no se puede bloquear ni ignorar, a diferencia de `SIGINT`— y el techo de 15 s es sólo eso, con tres mensajes distintos para los tres desenlaces. Cerrarla del todo exige que el instalador deje de bajar dentro de una sustitución de comandos, que es un cambio en `install.sh` y no en la prueba | Si falla, leer **cuál** de los tres mensajes salió: «nunca creó el directorio», «sigue corriendo 15 s después del SIGINT» o «salió y dejó el directorio». Sólo el tercero es un defecto del producto. Y no reintentar la suite entera, que es lo que esta deuda quiere evitar |
...
| Los chequeos de texto de v7 sobre la escalera son de mundo cerrado, pero su unidad es el bloque: una lista o un párrafo. Una afirmación repartida en dos párrafos sueltos de una sección de decisión («Un record que sobrevive a la sesión sigue importando.» / «Justo ahí aplica SDD.») no la ve ningún chequeo, porque un escaneo de texto no sigue un argumento de un párrafo a otro. Está declarada como residuo junto a `SDD_MEETS_FTD_STEMS` en `tests/test_flow_routing.py` | Una forma de fijar las secciones de decisión que no dependa de leer el argumento; por ejemplo, que esas secciones solo admitan ítems de lista y sus frases de introducción fijadas | Revisar a mano todo párrafo suelto nuevo en *Decision order*, *The routes*, *Ambiguous cases* o *Promotions* de `flow-applicability.md` |
| `ftd-procedure.md` se lee en cada FTD y no tiene techo de palabras, y el slice (d) le va a agregar la línea de Strict TDD | Medir después de d5 y fijar un techo con el mismo mecanismo que `FLOW_APPLICABILITY_WORD_CEILING` | Revisar su largo a mano en cada cambio: hoy tiene 880 palabras |
| `ProcedureIsLazyTest` es léxico: impide que un cuerpo que se carga siempre nombre `ftd-procedure.md` o su frase distintiva, pero no ve un cuerpo que parafrasee su contenido con otras palabras. Hoy ninguno lo hace | Ninguno barato: un chequeo de texto no reconoce una paráfrasis | Revisar a mano que los cuerpos que se cargan siempre no repitan el procedimiento |
| **Antes de la release de v7.** `README.md:96` presenta «primero entender, después especificar…» como la versión corta de cómo trabaja Pegasus, y `README.md:98` dice que `sdd-verify` es la autoridad final de readiness para cambios ejecutables o de configuración. Las dos contradicen v7: la mayoría de los cambios van por FTD, y fuera de SDD el readiness lo declara quien pidió el trabajo | Reescribir esas dos líneas como quedaron `docs/metodologia.md` y `MANUAL.md` en b6 | Nada: se corrigen antes de cortar la release |
| En *Roles y límites* de `docs/metodologia.md`, la fila nueva «Quien coordina un FTD» pone una obligación positiva en la columna *Límite importante*, donde las demás filas ponen un «No …» | Reescribirla como límite, si se encuentra una forma que no pierda la obligación | Nada |
| Los chequeos de mundo cerrado de v7 que filtran por una lista de raíces tuvieron un hueco de vocabulario en cada slice: *handoff*, *private key* y *API key*, *shippable*, *skip TDD*. Cada vez se amplió la lista después de que una revisión lo encontrara | Un corpus chico de frases adversarias por tema (readiness, secretos, rutas, modo TDD) contra el que se prueben esas listas antes de la revisión | Ampliar la lista con los sinónimos obvios del registro al escribir un chequeo nuevo |
### Deudas resueltas
