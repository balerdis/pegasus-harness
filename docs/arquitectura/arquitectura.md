...
| Tres residuos del corpus de vocabulario de los chequeos de mundo cerrado (`tests/test_closed_world_vocabulary.py`). Uno: el guard que deriva qué listas filtran un mundo cerrado solo ve un regex usado en la condición de una comprensión o como primer argumento de `assertFalse`/`assertIsNone`, así que un filtro escrito como un `for` con `if` no aparece (el docstring del módulo ya lo declara, y hoy todos los filtros son comprensiones). Dos: `CRAFT_RULE_SUBJECT` suma una raíz `config` muy general, que va a obligar a fijar cualquier mención nueva de «config» en `implementation-craft.md`. Tres: al tema *readiness* le faltan formas como «call it good», «good to merge» o «cleared for merge» | Enseñarle al guard los bucles `for` con `if`; acotar `config` a `openspec/config` o `config.yaml`; sumar esas frases al corpus y ampliar `READINESS_STEM` | Escribir los filtros nuevos como comprensiones, y ampliar el corpus cuando una revisión encuentre otra forma |
### Deudas resueltas
...
Queda una deuda sin resolver, anotada y no arreglada: `subAgentSessions` se llena desde el evento `session.created`, y el early-return de `chat.system.transform` asume que ese evento llega antes de que se arme el primer system prompt de la sesión hija. Es plausible pero no está verificado contra el orden real de despacho de eventos de OpenCode; si el supuesto falla, la inyección queda abierta para ese turno.
