...
| `ftd-procedure.md` se lee en cada FTD y no tiene techo de palabras, y el slice (d) le va a agregar la línea de Strict TDD | Medir después de d5 y fijar un techo con el mismo mecanismo que `FLOW_APPLICABILITY_WORD_CEILING` | Revisar su largo a mano en cada cambio. Medido después de d5: 915 palabras, y v7 no le agrega nada más |
