...
La retención guarda 5 generaciones, y no es un argumento de disco: el contenido son 80 archivos y 356 KB, el catálogo renderiza 89 archivos y 23 claves de configuración, así que un snapshot de reinstalación son unos 400 KB y cinco generaciones son unos 2 MB. Lo que la retención decide en realidad es cuánto atrás llega la promesa de recuperación.
...
|-------|-----------------|--------------------------|
| Un agente recibe `apply_patch` sin que Pegasus se la conceda nunca. El runtime agrupa `edit`, `write` y `apply_patch` bajo un único permiso `edit` al decidir qué herramientas manda (`Permission.disabled`: `const edits = ["edit", "write", "apply_patch"]`), y el comodín del mapa `tools` no lo alcanza porque ese filtro compara por clave exacta, no por patrón. Así que todo agente con `edit` o `write` —los trece— recibe la herramienta y su descripción imperativa, «Use the `apply_patch` tool to edit files». No es escalada: escribe exactamente lo que `edit` ya permite. Lo que rompe es la lectura del render, que presenta `"*": false` como si cerrara todo lo no nombrado. Ya costó una confusión real: un agente trató esa descripción como instrucción de mayor prioridad que el prompt de sistema y se negó a editar por SSH | Decidir qué se hace con una herramienta que no se puede quitar sin quitar `edit` y `write` con ella. Negarla por su propio nombre no sirve —el runtime nunca consulta esa clave— y denegar `edit` con patrón `*` es lo único que la saca, a costa de dejar al agente sin escribir. Las salidas son documentar el plegado en el propio render, para que nadie vuelva a leer el comodín como una clausura, o pedirle al runtime que evalúe `apply_patch` por su nombre. Mientras tanto el prompt compartido la nombra explícitamente, que ataca la confusión y no la causa | Si un agente cita la descripción de `apply_patch` para negarse a editar, decírselo en el pedido: el prompt compartido ya la nombra y no hay forma de sacarle la herramienta sin sacarle `edit` y `write` con ella. Y no leer el `"*": false` del render como si cerrara lo que no nombra |
...
| Un `granted_directories` inválido escrito a mano en el journal deja sin usar **todos** los comandos que lo leen, incluido el `directory revoke` con el que uno querría sacarlo. La validación al cargar es correcta y no se toca —impide el replay de una escalación— pero no hay camino de recuperación que no sea editar el JSON | Una decisión de diseño sobre la carga del journal: un modo que tolere un error sólo en ese campo, devolviéndolo vacío, le devolvería el comando de recuperación a la persona. Cambia la semántica de «cargar el journal», que es justo lo que no quisimos tocar en el apuro, así que merece su propia decisión. Por ahora el mensaje nombra el archivo y el campo, que es lo mínimo para arreglarlo a mano sin adivinar | El mensaje nombra el archivo y el campo: se corrige o se saca `granted_directories` del JSON a mano y vuelven todos los comandos. Para no llegar ahí, el campo se escribe con `pegasus directory grant`, nunca a mano |
| `test_install_script:InterruptedDownloadLeavesNoTempDirTest` es intermitente bajo carga: lanza un subproceso, le manda `SIGINT` al grupo y espera; con la máquina ocupada llega a agotar el tiempo. Aislado pasa siempre | Nada bloquea mirarlo, y conviene hacerlo antes de que alguien aprenda a reintentar la suite. El proyecto ya resolvió una carrera de esta familia una vez, dejando que el proceso asiente su salida en vez de competir con él, así que hay un precedente de forma: la espera tiene que depender de una condición observable y no de un plazo | Si falla sólo bajo carga, correrlo aislado antes de creerle — pero no enseñarle a nadie a reintentar la suite entera, que es lo que esta deuda quiere evitar |
| La poda de directorios no alcanza a ninguna instalación anterior a 5.28.0. `make_dir` sólo informa lo que **crea**, y en una instalación que ya existe los directorios ya están, así que `created_dirs` se queda vacío para siempre. Medido: un journal degradado a la forma anterior deja 31 directorios vacíos tras el uninstall, con `pruned` vacío. Y no hay curación: `uninstall` tampoco poda con el registro vacío, así que ni borrar y reinstalar sirve | Una decisión que no es del código: alcanzar la base instalada obliga a decidir qué hacer con un directorio del que no sabemos si es nuestro. Las dos salidas conocidas son adoptar todo directorio cuyos archivos estén todos reclamados por el journal —que no es saber quién lo creó sino que no hay nada ajeno adentro, y reintroduce la misma ambigüedad que la poda vino a cerrar— o preguntarle a la persona, informándolo en `doctor` y limpiándolo con un acto explícito. Lo segundo es lo honesto y es superficie nueva. Hasta entonces la poda vale hacia adelante y punto, que es mejor que borrar por las dudas | En una instalación anterior a 5.28.0, el `uninstall` deja directorios vacíos dentro del directorio de configuración del CLI. Se borran a mano sin riesgo: están vacíos, y nada de lo que Pegasus reclama queda adentro |
| La protección contra un cambio de esquema de `id` no cubre la forma `append`, y no tiene guardián general. Un `config-key` cuyo pointer termina en `/-` queda fuera del reconocimiento por dirección a propósito, porque varios registros comparten legítimamente la misma `(target, pointer)`; la consecuencia es que un append cuyo `id` cambia se retira y no se recrea en la misma corrida, y se cura en la siguiente. Y nada exige que un artefacto cuyo path se deriva de la identidad lleve el nombre en su `id`, que es la convención de la que depende toda la protección | Lo primero no es alcanzable con el contenido de hoy —los dos append embarcados tienen un solo hermano cada uno y sus `id` son literales fijos—, así que lo que falta es decidir si se cubre antes de que alguien agregue un tercero o se acepta que el primer append renombrado pierda una corrida. Lo segundo es un guardián derivado que hay que escribir: hoy el caso del prompt de sistema está cubierto por mutación, y el general no, así que un artefacto nuevo con path derivado e `id` fijo repite el defecto que 5.28.0 arregló | Si un `append` renombrado desaparece tras un `install`, correr `install` otra vez: se recrea en la corrida siguiente |
| El vocabulario de placeholders creció de dos nombres a siete, pero un cuerpo de contenido sólo puede recibir uno. `core/placeholders.py` afirma que agregar un nombre obliga a todo adapter a contestarlo, y `content` valida los cuerpos contra la lista entera, mientras `render._body` sólo contesta `skills_root`. Un autor que escriba `{{program_name}}` en el cuerpo de un agente pasa la validación de contenido y hace fallar el **install** | Elegir de qué lado se cierra la asimetría: o el render contesta en un cuerpo todo lo que el vocabulario declara, o el vocabulario se parte en dos —lo que puede pedir un asset y lo que puede pedir un cuerpo— y cada uno se valida contra el suyo. La segunda es más fiel a lo que pasa, porque las dos poblaciones son genuinamente distintas; la primera es más barata. La asimetría ya existía con un solo nombre y este cambio la multiplicó por cinco, que es lo que la volvió visible | Quien escriba contenido: en el cuerpo de un agente o de una skill, `{{skills_root}}` es el único placeholder que el render contesta. Los otros seis pasan la validación de contenido y hacen fallar el `install` |
| `test_tools_have_coverage` exige que algún archivo de test **nombre** cada script de `tools/`, no que lo ejercite, así que un test que lo mencione en un comentario lo satisface. Su docstring dice que habría hecho imposible que `build_catalog.py` se rompiera en silencio, y en rigor sólo habría exigido que alguien creara un archivo con ese nombre | Nada bloquea afinarlo: la forma derivada sería exigir que el script se ejecute dentro de algún test, no que su nombre aparezca. Lo que hay que decidir es cómo se observa eso sin volverlo frágil — instrumentar la ejecución es más caro que buscar una cadena, y un guardián que se rompe seguido enseña a ignorarlo, que es justo lo que la clasificación de `verify_darq` evita del otro lado | — |
| No hay CI en el repositorio: `.github/` no existe. La estructura documentada describe `manifests/` como "generados, verificados en CI", y esa verificación no la corre nada | Nada. Es una decisión sobre si el proyecto quiere una. El procedimiento de release es manual, está escrito en `docs/release-distribution.md`, y se ejecutó tal cual para 4.0.0, 4.1.0, 4.1.1, 4.1.2 y 5.0.0 | Antes de publicar, correr la suite completa a mano y seguir el procedimiento de `docs/release-distribution.md`, que es lo que hoy ocupa el lugar de la verificación automática |
| Las releases ya publicadas siguen llevando `skill-versiones-estandar-asi` dentro del artefacto, y con él la matriz de versiones homologadas de una organización. Son **51 tags** en un repositorio público —los anteriores a `fbbe94e`, el commit que los retiró del contenido el 2026-09-02—, más cada copia que alguien haya descargado. La cifra se mide con `git tag --no-contains fbbe94e` y no crece con las releases nuevas, que ya no lo llevan: decía 33 porque ese era el conteo el día que se escribió esta fila, y quedó vieja sin que nada avisara. Sacarlo de `main` no alcanza para deshacer eso | Una decisión sobre qué hacer con lo ya publicado, que es de quien administra el repositorio y no del código: no publicar nada, retirar los assets de las releases afectadas, o reescribir la historia. Cada opción tiene un costo distinto y ninguna es reversible | Quien haya descargado una release anterior a `fbbe94e` la tiene en disco: se borra la skill del directorio de skills instalado. Una instalación nueva ya no la trae |
| Un descriptor de MCP no tiene vocabulario cerrado: `_descriptor` sólo valida `name` y los placeholders del cuerpo, y `_refuse_foreign_form_fields` rechaza únicamente una clave que pertenezca al formulario de *otra* distribución (`_ALL_FORM_FIELDS`, la unión de `_FORM_FIELDS`). Una clave que no pertenece a ningún formulario —`witheld_tools` en vez de `withheld_tools`, por ejemplo— no está en esa unión, así que no la alcanza ningún chequeo: el descriptor carga limpio, `withheld_tools` queda en su default vacío, y el servidor no retiene nada mientras el front matter aparenta que sí | No es una línea de código que falte, es decidir qué tan permisivo es el formato: si el front matter de un descriptor es vocabulario cerrado (rechazar cualquier clave que ningún formulario reclama) o abierto como hoy. Lo primero exige enumerar de una vez toda clave válida por descriptor, no sólo por distribución | Quien escriba un descriptor propio: comparar las claves del front matter contra `_FORM_FIELDS` antes de confiar en él. Un typo no lo reporta nadie — el descriptor carga y el servidor no retiene nada |
| Las listas de inclusión aprobada de [docs/contrato-inclusion-artifacts.md](../contrato-inclusion-artifacts.md) se escriben y se actualizan a mano, y ya se atrasaron: el documento nombró tres servidores MCP mientras el contenido embarcaba cinco, y omitió dos plugins que se instalan, durante varios releases que igual se publicaron. El control de release exige demostrar que la lista distribuida coincide con la aprobada, y hoy eso se demuestra leyendo el mismo documento que habría que auditar | Una verificación que derive las listas del árbol —los descriptores de `src/pegasus/content/mcp/` y los assets de `src/pegasus/adapters/opencode/assets/plugins/`— y falle cuando difieran de lo aprobado en el contrato. No existe todavía, y por eso esto es deuda y no una limitación aceptada: lo que falta es un chequeo, no una decisión ya tomada | Cada cambio que agregue o saque un servidor o un plugin actualiza el contrato en el mismo commit, y la revisión lo mira. Un release que no lo hizo se detecta comparando a mano esas dos rutas contra la sección «Inclusión aprobada» |
