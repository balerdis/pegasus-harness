...
4. Desde un checkout del mismo tag RC, prepare un directorio de evidencia root-controlado y legible sólo por el verificador no-root `serg`; ejecute como root con rutas explícitas nuevas para staging y evidencia:
...
6. Repita el paso para los cinco perfiles con `--evidence-verifier serg`, usando el mismo directorio `root:<grupo-de-serg> 0750` fuera de `/home`. Cada JSON queda `root:<grupo-de-serg> 0640`: `serg` puede leer los cinco y no puede mutarlos. Cada registro debe contener `status: "PASS"`, el `profile` esperado, el journal y la misma identidad RC canónica: tag, nombre/SHA-256 del archive, SHA-256 del checksum, SHA-256 del manifest y `archive_root`.
7. Como `serg`, ejecute `python3 scripts/verify-v3-acceptance-matrix.py --rc-archive <archive> --rc-checksum <checksum> --release-manifest <manifest> --evidence-dir <directorio-root-controlado-fuera-de-home> --output-file <archivo-nuevo-en-directorio-serg-0700>`. La preparación anterior crea ese directorio de salida como `serg` con modo `0700`; no lo cree como root. El gate rechaza faltantes, duplicados, JSON inválido, perfiles fallidos, identidad RC diferente, paths inseguros y salida no privada. Solo su único `rc-acceptance-aggregate.json` con `status: "PASS"` habilita crear el tag anotado e inmutable `v3.1.0` sobre el mismo commit. No crea tags ni releases. Si falla cualquier paso, corrija en un commit nuevo y cree otro `v3.1.0-rc.N`; nunca mueva un tag RC ni el tag final.
