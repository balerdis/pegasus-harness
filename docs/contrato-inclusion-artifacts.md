...
4. Skills (`src/pegasus/content/skills/`, veinticuatro en total; `_shared/` no cuenta -- no es una skill invocable, es la biblioteca de fragmentos que las demás cargan):
   - Core: `skill-creator`, `skill-improver`, `skill-registry`, `project-agents-bootstrap`, `cognitive-doc-design`.
   - SDD: `sdd-apply`, `sdd-archive`, `sdd-design`, `sdd-explore`, `sdd-init`, `sdd-onboard`, `sdd-propose`, `sdd-spec`, `sdd-tasks`, `sdd-verify`.
   - Contexto: `session-context`, `session-handoff`.
   - Git y colaboracion: `branch-pr`, `chained-pr`, `comment-writer`, `issue-creation`, `work-unit-commits`. Ninguna con prefijo `sergio-` esta embarcada hoy; si alguna vez se agrega una, esta lista es la que se edita, no una regla que la excluya sola.
   - Quality, security y operations: `lazy-load-prompt-audit`.
   - Domain: ninguna. `skill-versiones-estandar-asi` y `laravel-security` se retiraron del contenido embarcado: el primero codifica el estándar de tecnología interno de una organización y el segundo llegó por la misma mudanza, y ninguno de los dos es guía genérica que corresponda que embarque un producto público.
   - Sin categoria fija: `api-service-contract-documentation`.
...
Este documento ya no las repite: viven en un solo lugar, junto con su motivo. Una edicion a mano sobre un artifact de Pegasus no sobrevive al proximo `install` y `uninstall` la borra, los `append` quedan fuera del aviso `overwritten` por construccion y `uninstall` no tiene aviso equivalente — el porque de cada una y la recuperacion por snapshot, con su ventana de veinte generaciones, estan en [Limitaciones aceptadas](arquitectura/arquitectura.md#limitaciones-aceptadas). De las listas de "Inclusion aprobada", los servidores MCP, los plugins de OpenCode y las skills ya no se mantienen a mano sin control: `tests/test_inclusion_contract.py` deriva del arbol los servidores en `src/pegasus/content/mcp/*.md`, los plugins en `src/pegasus/adapters/opencode/assets/plugins/` y las skills en `src/pegasus/content/skills/` (excepto `_shared/`, que no es una skill invocable), y los compara contra los nombres que esta seccion declara -- pero no de la misma forma para los tres. Para los plugins la comparacion es realmente bidireccional: un archivo de mas o de menos, en el arbol o en el documento, hace fallar el test, porque un nombre de plugin se reconoce por su extension (`.ts`/`.js`), no por pertenecer ya al arbol. Para los servidores MCP y las skills, un nombre solo cuenta como tal si coincide exactamente con un id o un nombre que el arbol ya tiene -- de otro modo la prosa libre de la seccion (`download`, `npm`, `remote`, una ruta, o el bullet que nombra a proposito dos skills retiradas) contaria como si fuera un artifact -- asi que el test solo prueba con certeza que el arbol no tenga mas de lo que el documento nombra. La direccion contraria para esos dos -- que el documento no nombre algo que el arbol ya no tiene -- la cierran las tres cifras tipeadas mas abajo ("cinco" servidores, "seis" plugins, "veinticuatro" skills), derivadas del mismo arbol: sacar un item sin tocar el documento hace que la cifra deje de coincidir. Lo que ni la comparacion de nombres ni la cifra detectan es que el documento nombre, ademas de todos los reales, un id o una skill que nunca existio: ese nombre se descarta antes de comparar, por el mismo filtro que evita que la prosa libre se cuente como artifact, y no cambia ninguna cifra. Esto exigio dejar de describir las skills por categoria y excepcion y enumerarlas por nombre, igual que los MCP y los plugins: una regla categorica ("todos los de contexto, excepto los que empiecen con sergio-") no tiene una forma de comparacion contra el arbol que no reimplemente la categoria a mano en el test, que es la misma clase de mantenimiento manual sin control que esto busca cerrar. Ademas, el test verifica que a quien esta seccion describe "sin prefijo" o "con prefijo" coincida con `_RENAMED_ASSETS` (`src/pegasus/adapters/opencode/adapter.py`). Lo que sigue sin verificacion automatica es que la salida realmente distribuida en un release coincida con lo que el arbol fuente contiene; eso sigue figurando en [Deudas sin unidad asignada](arquitectura/arquitectura.md#deudas-sin-unidad-asignada) con lo que hay que hacer en cada release hasta que exista.
...
De estos puntos, `tests/test_inclusion_contract.py` prueba que los plugins de OpenCode nombrados en la inclusion aprobada coincidan, en las dos direcciones, con lo que el arbol fuente contiene; que los servidores MCP y las skills que el arbol contiene esten todos nombrados alli, mas que las tres cifras tipeadas para servidores, plugins y skills coincidan con el conteo del arbol (lo que cierra, para esos dos, la direccion de que el documento no se haya quedado nombrando algo que el arbol ya no tiene); y que el prefijo declarado para cada plugin coincida con `_RENAMED_ASSETS`. Un id de servidor o una skill que el documento nombre de mas, sin haber existido nunca en el arbol, no lo detecta ninguno de esos dos mecanismos: se filtra antes de comparar, igual que la prosa libre de la seccion. Sigue demostrandose a mano, sin una prueba que lo derive, que la lista efectivamente distribuida en un release (el binario `pegasus` publicado) coincida con la inclusion aprobada. Eso esta anotado como deuda en [Deudas sin unidad asignada](arquitectura/arquitectura.md#deudas-sin-unidad-asignada).
