...
| `install` decide "already current" comparando lo deseado contra el **disco**, mientras `doctor` reporta drift comparando el disco contra el digest guardado en el **journal**. Las dos respuestas se contradicen cuando una edición a mano produce exactamente lo que una versión posterior va a renderizar: `install` no escribe, el journal se queda con el digest viejo, y `doctor` reporta drift para siempre — cada corrida siguiente toma la misma decisión. Apareció instalando 5.5.1 sobre una instalación parchada a mano con el mismo contenido, y hoy se sale reviertiendo el parche antes de instalar. No hay `--force` | Una decisión sobre cuál de las dos preguntas es la verdadera: o `install` reconcilia el journal cuando el disco ya coincide con lo deseado, o `doctor` toma "disco == deseado" como al día sin mirar el digest registrado |
...
| No hay `pegasus --version`. La versión sólo se puede leer corriendo `pegasus doctor --json` y sacando `pegasus_version`, que es un comando de diagnóstico y no una pregunta por el binario | Nada. Es una decisión de interfaz de una línea |
