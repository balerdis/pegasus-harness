...
| El skill `skill-versiones-estandar-asi` no es guía genérica: codifica el estándar de tecnología interno de una organización, incluida una matriz de versiones homologadas que sólo tiene sentido y sólo es correcta dentro de esa organización. Viaja en el contenido embarcado desde el commit de la demolición de v3, así que está en todos los releases publicados de v4 y v5, y dentro del artefacto que baja cualquiera. Nadie decidió publicarlo: viajó con la mudanza de contenido | Una revisión de qué corresponde que embarque un producto público, y una decisión sobre los releases que ya salieron, que sacarlo de `main` no alcanza para deshacer. Mientras no se decida, cada release nuevo lo sigue llevando. Hay un segundo caso a mirar en la misma revisión: `laravel-security`, que también llegó por esa mudanza |
...
| Dos defectos de `codebase-memory-mcp` observados en instalaciones reales y nunca reportados a DeusData: (a) un archivo PHP sintácticamente válido con HTML embebido después de `?>` produce `parse_partial` de archivo entero —909 casos de código propio en un solo repo, y el mismo patrón aparece en un `.template.sql` con placeholders—; (b) un repositorio con checkouts git anidados falla con un `Pipeline failed` genérico que no nombra la causa | Nada nuestro: son de otro producto. Lo que falta es abrir los dos issues con el caso mínimo de cada uno. Mientras no se reporten, cada instalación los vuelve a descubrir sola |
| El proyecto `ospreviene` no está indexado en CBM a nivel raíz; sus subproyectos sí. Es el caso (b) de arriba visto desde adentro: la raíz contiene los checkouts anidados que hacen fallar el pipeline | Que se resuelva (b), o una exclusión que saque los checkouts anidados del alcance del índice de la raíz |
| Quedan librerías de terceros de un solo archivo indexadas en `leannec-backend` —`class.phpmailer.php`, `class.smtp.php`, `class.pop3.php`— que el `.cbmignore` no excluye. Se dejaron a propósito: pueden ser copias modificadas, y excluir una copia que el proyecto tocó esconde código que sí es suyo | Mirar cada una contra su original y decidir de a una. No es una regla que se pueda escribir sin abrir los archivos |
| Adoptar un artefacto que ya existía descarta los permisos que la persona le haya agregado a mano: el render escribe el mapa entero, no lo funde con lo que había | Nada. Necesita decidir qué gana cuando el producto y la persona declaran cosas distintas sobre la misma clave, que es una pregunta de contrato y no un merge |
| `doctor` no puede contrastar la versión de un servidor **atado** con la que el descriptor fija, porque el `serverInfo` del handshake `initialize` no es confiable: engram reporta `0.1.0` corriendo `1.20.0`. Un servidor atado es justamente el caso donde la versión no la controla Pegasus | Una fuente de versión que no sea el propio servidor, o servidores que digan la verdad. Mientras tanto, atar es una decisión informada de quien la toma |
| El comodín `cbm*` con el que se conceden las herramientas de un servidor MCP concede también las destructivas que ese servidor expone — `delete_project`, `index_repository`, `ingest_traces` — a los dos agentes primarios. La granularidad del comodín es el servidor, no la herramienta | Una forma de conceder por herramienta que no obligue a enumerar en el descriptor todo lo que un servidor expone, que es exactamente lo que el comodín existe para evitar |
