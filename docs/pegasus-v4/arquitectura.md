...
| El campo `tools`, donde se renderiza el deny de cada agente, está deprecado en el schema de ese CLI a favor de `permission`. Hoy funciona: el runtime lo traduce a reglas de `permission` al cargar la configuración | Migrar el render de `tools` a `permission`. Lo que la migración tiene que preservar quedó verificado en runtime contra el render viejo, con un agente real: un prefijo de MCP concedido le gana al deny general, y un servidor conectado cuyo prefijo no se concedió no aparece siquiera en la lista de herramientas del agente. Esa verificación es el criterio de aceptación de la migración, no su prueba: hay que volver a correrla contra el render nuevo |
