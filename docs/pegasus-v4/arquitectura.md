...
| Dos defectos de `codebase-memory-mcp` reproducidos en instalaciones reales y todavía sin reportar a DeusData: (a) un archivo PHP sintácticamente válido con HTML embebido después de `?>` produce `parse_partial` de archivo entero —909 casos de código propio en un solo repo, y el mismo patrón en un `.template.sql` con placeholders—; (b) un directorio que el usuario que indexa no puede leer hace fallar todo el pipeline con un `Pipeline failed` cuyo texto apunta al lado equivocado, en vez de saltear ese directorio como saltea cualquier otro excluido | Nada nuestro: son de otro producto, y los dos tienen repro mínimo. Falta abrir los issues. Mientras no se reporten, cada instalación los vuelve a descubrir sola |
...
| `doctor --start-mcp-servers` no decía nada de un servidor atado. Peor: como un servidor atado no escribe clave `/mcp/<id>`, una instalación cuyos servidores están todos atados reportaba «No MCP servers configured» — no un hueco en el reporte sino una afirmación falsa sobre la máquina | Una entrada de convención sin clave de configuración al lado es exactamente la forma que deja una atadura, y alcanza para decir la verdad en su lugar: el servidor se reporta con estado `bound` y la explicación de que lo administra la persona. Lo que no se dice es a qué clave se lo ató, porque el journal nunca lo registró y un reporte que lo adivinara sería la misma clase de falsedad que esto vino a sacar. Arrancarlo sigue fuera de alcance por el mismo motivo, y contrastar su versión sigue bloqueado por afuera: el `serverInfo` del handshake no es confiable, engram reporta `0.1.0` corriendo `1.20.0` |
