...
| El campo `tools`, donde se renderiza el deny de cada agente, está deprecado en el schema de ese CLI a favor de `permission`. Hoy sigue funcionando: el runtime lo traduce a reglas de `permission` al cargar la configuración, así que la deprecación es real pero no urgente | Migrar el render de `tools` a `permission`. Verificarlo exige una sesión con proveedor conectado; leer el código fuente no alcanza para probar que la migración quedó bien, sólo para diseñarla |
