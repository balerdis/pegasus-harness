...
| El plugin de engram que se embarca está inerte —seis líneas— y su comentario afirma que «el MCP provee la integración». Es cierto sólo para el texto del protocolo, que un servidor MCP puede entregar por su campo de instrucciones. No es cierto para lo que el plugin de la línea 3.x hacía y un MCP no puede: registrar `chat.message` (una vez por mensaje del usuario, antes de que el modelo lo vea), `tool.execute.after` (después de cada herramienta, que es la captura pasiva), los eventos de ciclo de vida de sesión, la supresión de sesiones de sub-agente, y hablar con un `engram serve` local. El archivo tiene un solo commit —el de la demolición de v3—, así que nunca fue una migración: fue un stub con una justificación escrita al lado | Decidir qué parte de eso vuelve. Si vuelve el plugin, vuelve como activo embarcado del adapter de OpenCode y necesita un test que corra desde el zip. Si no vuelve, el comentario tiene que dejar de afirmar una cobertura que no existe, y `doctor` debería poder decir que la captura automática no está |
