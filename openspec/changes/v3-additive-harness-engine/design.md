...
| MCP integrity/remediation | Store verified Engram metadata. Playwright is fixed to `@playwright/mcp@0.0.79` plus `playwright`/`playwright-core@1.63.0-alpha-2026-08-05`, with committed explicit `https://registry.npmjs.org/` resolved URLs and SRI for every locked package. Run `npm ci --ignore-scripts` only in fresh Pegasus staging with registry/proxy/mirror environment and user npm config isolated; verify lock/tree, atomically promote, then direct-entrypoint probe. Existing MCPs become links only after exact config shape, resolved executable, `--version`, and required probe pass. | Placeholder/floating metadata, inherited mirror/proxy registry, lifecycle scripts, `npx`, unverified config, reinstall/adoption/removal. |
...
| RC acceptance laboratory | `scripts/provision-v3-rc-host.sh` is the test-only destructive host emulator: it recreates only the profile-mapped dedicated user and installs fixed Node `24.15.0` and OpenCode `1.18.13`. `scripts/accept-v3-isolated.sh` is the only test-only orchestrator: explicit `--profile`, `--rc-archive`, checksum, manifest, and exact `--confirm-recreate-user` are required before preflight and provision; root then copies the verified payload below `/var/lib/pegasus-acceptance/<target>/rc`, validating every root-owned non-writable ancestor. The payload stays root-owned and grants the target group only read/execute access. | No target-controlled handoff ancestor or payload replacement, world-readable payload, unsafe archive modes/handoff paths, Pegasus host bootstrap, catalog entry, owned user artifact, or runtime product behavior. |
...
Unjournaled content is user-owned. Plans expose id, target/key, action, reason, exact artifact metadata, CBM provenance/output SHA, Context7 endpoint/no-integrity state, and browser state. Playwright staging is registry-isolated; any verify/install/probe failure removes temp/created dependency paths, skips dependent config/journal, and preserves user content.
...
`CbmProvenance = {repository, tag, commit, tree, platform, builder_image_digest, build_command_sha256, output_path, output_sha256}`. `PlaywrightLock = {registry:"https://registry.npmjs.org/", mcp:"0.0.79", playwright:"1.63.0-alpha-2026-08-05", entries:[{resolved,integrity}]}`. `DependencyEntry` contains exact metadata/runtime argv; `McpLink` adds `{resolved_path, probe_result, ownership:false}`. Journal remains target-owned; malformed/missing state is unowned.

## Testing Strategy

| Layer | What to Test | Approach |
|---|---|---|
| Unit | Fixed Playwright graph/SRIs/npmjs isolation, safe archive members, links, Context7 | RED rejects placeholders, lock drift, non-npmjs resolved URL, inherited proxy/mirror/user npm config, lifecycle scripts, failed probes, remote version/integrity claims. |
