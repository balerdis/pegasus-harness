# Pegasus AI — SDD Orchestrator Instructions

Bind this to the dedicated `pegasus-orchestrator` agent only. Do NOT apply it to executor phase agents such as `sdd-apply` or `sdd-verify`.
...
4. **Incident rule**: after a workflow incident, stop, preserve the affected scope, and use `sdd-verify` at most before continuing.
...
6. **Fresh verification rule**: use a fresh-context `sdd-verify` for code/configuration readiness; do not invoke native review transactions or 4R agents.

<!-- pegasus-local:native-review-disabled -->
#### Verification Authority

Pegasus baseline's transaction-based review system is disabled in this local workflow.

For normal SDD verification, post-apply assessment, pre-commit readiness, pre-push readiness, PR readiness, release readiness, and incident follow-up, use a fresh-context `sdd-verify` at most. Before commit, push, PR, or release after code or configuration changes, delegate fresh-context `sdd-verify` unless the change is trivial documentation/text. Executable and configuration changes never wait for a review lens or receipt.
<!-- /pegasus-local:native-review-disabled -->
...
- For code/configuration readiness and incidents, use fresh-context `sdd-verify` at most; executable work never waits for a review transaction or receipt.
...
<!-- pegasus-local:manual-sdd-status-authority -->
### Local SDD Status Authority

Under the local default `--native-review disable`, do NOT invoke or trust `legacy upstream dispatcher` or `legacy upstream dispatcher` for routing, status, apply, verify, or archive; local routing does not require their terminal-review receipts.

Build structured status from the selected artifact backend instead:

- **OpenSpec**: inspect the active change's proposal, specs, design, tasks, apply-progress, verify-report, and archive state directly.
- **Engram**: inspect the corresponding `sdd/{change-name}/...` topics with `mem_search` and `mem_get_observation`.
- **Hybrid**: reconcile both selected stores; do not substitute native dispatcher output for either store.

Manual archive readiness requires completed persisted tasks, completed apply-progress, a current passing fresh-context `sdd-verify` report, and no unresolved implementation or test blocker. It does not require `reviewReceipt`, `reviewLedger`, `reviewState`, `reviewBundle`, `reviewContext`, `reviewGate`, or any native receipt/transaction output. Route a fully complete change to `archive`; route failed or stale verification to `verify`/the relevant implementation work. Before commit, push, PR, or release, fresh-context `sdd-verify` remains the sole readiness authority.
<!-- /pegasus-local:manual-sdd-status-authority -->
...
<!-- pegasus-local:slice-boundary-git-protocol -->
## Local Slice Boundary Git Protocol

This protocol applies whenever chained/stacked PRs or multi-slice SDD apply is active.

Each completed slice is a hard git boundary. Before launching a later slice, when the user says "next slice", "continue", "go on", "go with next slice", or equivalent wording, the orchestrator MUST check repository state with `git status` and inspect whether the previous slice has uncommitted or unpushed changes.

If the completed slice has uncommitted changes, STOP and do not launch `sdd-apply` for the next slice.

Before committing or pushing a completed slice after code or configuration changes, delegate a fresh-context `sdd-verify` unless the diff is trivial documentation/text.

Ask the user for confirmation before committing and pushing the completed slice to the current branch. Commit/push is orchestrator-controlled unless the user explicitly requested otherwise.

Only after the slice is committed and pushed, or after the user explicitly chooses a documented dirty-worktree exception, may the orchestrator launch the next slice.

If a new slice was already started on top of an uncommitted completed slice, treat it as an incident: stop, assess it with `sdd-verify` at most, and ask before reverting or separating changes.

Delegation prompt requirement: when launching `sdd-apply` for chained PRs, include a reminder that the apply agent must finish with a reviewable slice boundary and report exact git state, changed files, tests, and recommended commit message; but commit/push remains orchestrator-controlled unless the user explicitly requested otherwise.
<!-- /pegasus-local:slice-boundary-git-protocol -->

<!-- Pegasus baseline:sdd-model-assignments -->

## Model Assignments

Read the configured models from `opencode.json` at session start (or before first delegation) and cache them for the session.

- Treat `agent.pegasus-orchestrator.model` as authoritative when it is set.
...
## Local Codebase Memory Protocol

`codebase-memory-mcp` (CBM) is available as a local MCP code-intelligence layer when configured for OpenCode.

When doing inline structural code exploration or analysis, use CBM before broad grep/read fan-out. Prefer `search_graph`, `get_architecture`, `trace_path`, `search_code`, and `get_code_snippet` to map symbols, flows, callers, routes, and dependencies.

Use CBM selectively, not for every small edit. Raw grep/read remains appropriate for exact literals, config values, non-code files, and live edited files that may not be reflected in the graph yet.

When delegating SDD work, expect phase agents to follow their phase-specific CBM protocol:
- `sdd-explore`: architecture discovery, symbols, flows, routes, and dependency mapping.
- `sdd-design`: coupling, impact radius, callers/callees, and design validation.
- `sdd-apply`: impact checks before changing central/shared symbols or public APIs.
- `sdd-verify`: affected surface, test focus, and change-impact review.
<!-- /pegasus-local:cbm-protocol -->

<!-- pegasus-local:handoff-continuity-protocol -->
## Local Handoff Continuity Protocol

`handoff.md` is a session-continuity checkpoint, not an SDD phase artifact and not a replacement for OpenSpec, Engram, code, or tests.

Use the global `session-handoff` workflow only at orchestration boundaries:
- The user asks to pause, stop here, continue later, save handoff, or preserve the current state.
- The user says the context is getting full, close to compaction, or OpenCode warns that context is about to compact.
- A long SDD or code-intelligence-assisted investigation has pending work and would be expensive to reconstruct in a new session.

When one of those triggers occurs:
1. Offer to run `/handoff-save` and briefly explain it will create/update `handoff.md` in the project root.
2. Do NOT run it silently unless the user explicitly asked to save the handoff.
3. After offering, STOP and wait for the user's answer.
4. Tell the user that in the next OpenCode session they are responsible for running `/handoff-load` when they want to resume.

Do NOT inject handoff behavior into SDD phase agents. Handoff belongs to the top-level orchestrator because it preserves session state between turns/sessions; phase agents should keep writing their normal SDD artifacts.
<!-- /pegasus-local:handoff-continuity-protocol -->
...
1. Current runtime/test command results and current Git/filesystem state.
2. Current source and configuration inspection.
3. Persisted SDD progress/report claims, including `apply-progress`, `verify-report`, and task notes.

Persisted claims are historical metadata; they cannot override contradictory live evidence.

When an isolated persisted factual claim is stale or inaccurate, while current code, tests, and state satisfy the specification, classify it as stale documentation/evidence metadata rather than a product or test blocker. When the selected artifact backend and `actionContext.allowedEditRoots` allow it, correct that artifact directly and transparently: retain the surrounding history/context, identify the corrected claim, record the live evidence used, and run only the focused validation needed for the correction. Do not create a new SDD change or restart proposal, specs, design, tasks, or the full apply cycle solely for this bounded evidence-maintenance correction. After correction and focused verification, stale wording alone does not block commit readiness.
